...
{
    public partial class IcParcaIsEmirleri : Page
    {
                ...
                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["SqlDB"].ConnectionString))
                {
                    string query = @"
                        SELECT 
                            ...
                            ie.Grami,
                            ie.KalipNo,
                            ISNULL(ie.IcParcaDurum, 'YENİ') AS IcParcaDurum,
                            ie.OlusturmaTarihi
                        FROM IsEmirleri ie
                        WHERE ie.IcParca = 1 
                            AND ie.Durum = 'ACTIVE'
                        ORDER BY 
                            CASE ISNULL(ie.IcParcaDurum, 'YENİ')
                                WHEN 'YENİ' THEN 1
                                WHEN 'AKTİF' THEN 2
        ...
        protected void rptIsEmirleri_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                string durum = row["IcParcaDurum"].ToString();
        ...
        private void GuncelleDurum(int isEmriId, string yeniDurum)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["SqlDB"].ConnectionString))
                {
                    string query = @"
                        UPDATE IsEmirleri 
                        SET IcParcaDurum = @IcParcaDurum
                        WHERE IsEmriId = @IsEmriId";

                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@IsEmriId", isEmriId);
                        cmd.Parameters.AddWithValue("@IcParcaDurum", yeniDurum);
